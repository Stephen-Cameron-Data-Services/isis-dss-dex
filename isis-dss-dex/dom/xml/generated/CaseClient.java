//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.4-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2015.12.29 at 11:58:37 AM EST 
//


package generated;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.JAXBElement;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlElementRef;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for CaseClient complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="CaseClient">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="ClientId" type="{}NonEmptyString"/>
 *         &lt;element name="ReferralSourceCode" type="{}NonEmptyString" minOccurs="0"/>
 *         &lt;element name="ReasonsForAssistance" minOccurs="0">
 *           &lt;complexType>
 *             &lt;complexContent>
 *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                 &lt;sequence>
 *                   &lt;element name="ReasonForAssistance" type="{}ReasonForAssistance" maxOccurs="unbounded"/>
 *                 &lt;/sequence>
 *               &lt;/restriction>
 *             &lt;/complexContent>
 *           &lt;/complexType>
 *         &lt;/element>
 *         &lt;element name="ExitReasonCode" type="{}NonEmptyString" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "CaseClient", propOrder = {
    "clientId",
    "referralSourceCode",
    "reasonsForAssistance",
    "exitReasonCode"
})
public class CaseClient {

    @XmlElement(name = "ClientId", required = true)
    protected String clientId;
    @XmlElementRef(name = "ReferralSourceCode", type = JAXBElement.class, required = false)
    protected JAXBElement<String> referralSourceCode;
    @XmlElementRef(name = "ReasonsForAssistance", type = JAXBElement.class, required = false)
    protected JAXBElement<CaseClient.ReasonsForAssistance> reasonsForAssistance;
    @XmlElementRef(name = "ExitReasonCode", type = JAXBElement.class, required = false)
    protected JAXBElement<String> exitReasonCode;

    /**
     * Gets the value of the clientId property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getClientId() {
        return clientId;
    }

    /**
     * Sets the value of the clientId property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setClientId(String value) {
        this.clientId = value;
    }

    /**
     * Gets the value of the referralSourceCode property.
     * 
     * @return
     *     possible object is
     *     {@link JAXBElement }{@code <}{@link String }{@code >}
     *     
     */
    public JAXBElement<String> getReferralSourceCode() {
        return referralSourceCode;
    }

    /**
     * Sets the value of the referralSourceCode property.
     * 
     * @param value
     *     allowed object is
     *     {@link JAXBElement }{@code <}{@link String }{@code >}
     *     
     */
    public void setReferralSourceCode(JAXBElement<String> value) {
        this.referralSourceCode = value;
    }

    /**
     * Gets the value of the reasonsForAssistance property.
     * 
     * @return
     *     possible object is
     *     {@link JAXBElement }{@code <}{@link CaseClient.ReasonsForAssistance }{@code >}
     *     
     */
    public JAXBElement<CaseClient.ReasonsForAssistance> getReasonsForAssistance() {
        return reasonsForAssistance;
    }

    /**
     * Sets the value of the reasonsForAssistance property.
     * 
     * @param value
     *     allowed object is
     *     {@link JAXBElement }{@code <}{@link CaseClient.ReasonsForAssistance }{@code >}
     *     
     */
    public void setReasonsForAssistance(JAXBElement<CaseClient.ReasonsForAssistance> value) {
        this.reasonsForAssistance = value;
    }

    /**
     * Gets the value of the exitReasonCode property.
     * 
     * @return
     *     possible object is
     *     {@link JAXBElement }{@code <}{@link String }{@code >}
     *     
     */
    public JAXBElement<String> getExitReasonCode() {
        return exitReasonCode;
    }

    /**
     * Sets the value of the exitReasonCode property.
     * 
     * @param value
     *     allowed object is
     *     {@link JAXBElement }{@code <}{@link String }{@code >}
     *     
     */
    public void setExitReasonCode(JAXBElement<String> value) {
        this.exitReasonCode = value;
    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="ReasonForAssistance" type="{}ReasonForAssistance" maxOccurs="unbounded"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "reasonForAssistance"
    })
    public static class ReasonsForAssistance {

        @XmlElement(name = "ReasonForAssistance", required = true)
        protected List<ReasonForAssistance> reasonForAssistance;

        /**
         * Gets the value of the reasonForAssistance property.
         * 
         * <p>
         * This accessor method returns a reference to the live list,
         * not a snapshot. Therefore any modification you make to the
         * returned list will be present inside the JAXB object.
         * This is why there is not a <CODE>set</CODE> method for the reasonForAssistance property.
         * 
         * <p>
         * For example, to add a new item, do as follows:
         * <pre>
         *    getReasonForAssistance().add(newItem);
         * </pre>
         * 
         * 
         * <p>
         * Objects of the following type(s) are allowed in the list
         * {@link ReasonForAssistance }
         * 
         * 
         */
        public List<ReasonForAssistance> getReasonForAssistance() {
            if (reasonForAssistance == null) {
                reasonForAssistance = new ArrayList<ReasonForAssistance>();
            }
            return this.reasonForAssistance;
        }

    }

}
